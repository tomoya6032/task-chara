class ActivitiesController < ApplicationController
  ...
  def generate_daily_report_from_chat(conversation_history)
    ...
    messages = [
      ...
      { role: "user", content: "以下のAI秘書との会話履歴を基に、4つの項目（①本日の訪問内容、②課題や修正点、③今後の方向性、④その他）をすべて含む日報を500〜800文字程度で作成してください。マークダウン記号は使わず、項目名を段落の頭に明記し、自然で温かみのある丁寧語で記述してください。\n\n【会話履歴】\n#{chat_context}" }
    ]
  ...
  def build_daily_report_generation_prompt
    <<~PROMPT
      あなたは訪問介護・福祉現場の日報作成アシスタントです。AI秘書との会話履歴から、業務や活動に関連する情報を抽出し、親しみやすく分かりやすい自然な文章（丁寧語・ですます調）で日報として整理してください。マークダウン記号（#や##、-など）は一切使用せず、段落の頭に項目名を明記してください。大学生スタッフやチームメンバーが一読して状況が理解できるように、温かみのある表現で記述してください。

      【日報の構成】
      日報は必ず以下の4つの項目で構成し、全体の文字数は500〜800文字の範囲内（600文字程度が目安）に収めてください。マークダウン記号は一切使わず、項目名を段落の頭に明記してください。

      ① 本日の訪問内容
      会話履歴の概要を、大学生でも一読して状況が理解できるレベルに分かりやすく修正・要約して記載してください。

      ② 課題や修正点
      会話内容から見えてきた課題、今後修正や確認が必要な点があれば、それらを具体的に書き残してください。特になければ「特になし」と記載してください。
      ...
      【注意事項】
      - 会話の文脈から業務性質を推測し、4つの項目すべてを含む日報を500〜800文字程度で作成してください
      - 個人情報や機密性の高い内容は適切に匿名化してください
      - 会話にない情報は推測せず、自然な範囲で記述してください
      - マークダウン記号は使わず、自然で温かみのある丁寧語で記述してください
    PROMPT
  end
